...
class listaRetencionEmitidaController extends Controller
{
    public function vista()
    {
        try{
            ...
            return view('admin.sri.listaRetencionesEmitidas.index',['sucursal'=>$sucursal,'gruposPermiso'=>$gruposPermiso, 'PE'=>Punto_Emision::puntos()->get(),'permisosAdmin'=>$permisosAdmin]);
        }
        catch(\Exception $ex){      
    ...
    {  
        try{        
            ...
            return view('admin.sri.listaRetencionesEmitidas.index',['idsucursal'=>$request->get('sucursal'),'fecha_desde'=>$request->get('idDesde'),'fecha_hasta'=>$request->get('idHasta'),'sucursal'=>$sucursal,'retencionCompras'=>$retencionCompras,'gruposPermiso'=>$gruposPermiso, 'PE'=>Punto_Emision::puntos()->get(),'permisosAdmin'=>$permisosAdmin]);
        }
        catch(\Exception $ex){      
            return redirect('inicio')->with('error2','Ocurrio un error en el procedimiento. Vuelva a intentar. ('.$ex->getMessage().')');
        }
