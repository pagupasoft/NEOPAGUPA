...
class listaRetencionRecibidaController extends Controller
{
    ...
    {  
        ...
        try{        
            ...
            return view('admin.sri.listaRetencionesRecibidas.index',['fechaActual'=>$fechaActual,'fecha_desde'=>$request->get('idDesde'),'fecha_hasta'=>$request->get('idHasta'),'sucursal'=>$sucursal,'retencionVentas'=>$retencionVentas,'gruposPermiso'=>$gruposPermiso, 'PE'=>Punto_Emision::puntos()->get(),'permisosAdmin'=>$permisosAdmin]);
        }
        catch(\Exception $ex){      
            return redirect('inicio')->with('error2','Ocurrio un error en el procedimiento. Vuelva a intentar. ('.$ex->getMessage().')');
        }
