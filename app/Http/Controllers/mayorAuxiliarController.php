...
class mayorAuxiliarController extends Controller
{
    ...
    $permisosAdmin=DB::table('usuario_rol')->select('permiso_ruta', 'permiso_nombre', 'permiso_icono', 'tipo_id', 'grupo_id', 'permiso_orden')->join('rol_permiso','usuario_rol.rol_id','=','rol_permiso.rol_id')->join('permiso','permiso.permiso_id','=','rol_permiso.permiso_id')->where('permiso_estado','=','1')->where('usuario_rol.user_id','=',Auth::user()->user_id)->orderBy('permiso_orden','asc')->get();
            ...
            return view('admin.contabilidad.mayorAuxiliar.index',['ini'=>$request->get('cuenta_inicio'),'fin'=>$request->get('cuenta_fin'),'sucursalC'=>$request->get('sucursal_id'),'sucursales'=>Sucursal::sucursales()->get(),'fDesde'=>$request->get('fecha_desde'),'fHasta'=>$request->get('fecha_hasta'),'datos'=>$datos,'cuentas'=>Cuenta::Cuentas()->orderBy('cuenta_numero','asc')->get(),'cuentaFinal'=>Cuenta::CuentasDesc()->first()->cuenta_id,'PE'=>Punto_Emision::puntos()->get(),'tipoPermiso'=>$tipoPermiso,'gruposPermiso'=>$gruposPermiso, 'permisosAdmin'=>$permisosAdmin]);      
        }catch(\Exception $ex){
