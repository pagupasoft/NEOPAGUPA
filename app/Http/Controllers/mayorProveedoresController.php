...
class mayorProveedoresController extends Controller
{
    ...
    $permisosAdmin=DB::table('usuario_rol')->select('permiso_ruta', 'permiso_nombre', 'permiso_icono', 'tipo_id', 'grupo_id', 'permiso_orden')->join('rol_permiso','usuario_rol.rol_id','=','rol_permiso.rol_id')->join('permiso','permiso.permiso_id','=','rol_permiso.permiso_id')->where('permiso_estado','=','1')->where('usuario_rol.user_id','=',Auth::user()->user_id)->orderBy('permiso_orden','asc')->get();
            return view('admin.contabilidad.mayorProveedores.index',['cuentas'=>Cuenta::CuentasMovimiento()->get(),'proveedores'=>Proveedor::proveedores()->get(),'PE'=>Punto_Emision::puntos()->get(),'tipoPermiso'=>$tipoPermiso,'gruposPermiso'=>$gruposPermiso, 'permisosAdmin'=>$permisosAdmin]);
        }
        catch(\Exception $ex){      
    ...
    $permisosAdmin=DB::table('usuario_rol')->select('permiso_ruta', 'permiso_nombre', 'permiso_icono', 'tipo_id', 'grupo_id', 'permiso_orden')->join('rol_permiso','usuario_rol.rol_id','=','rol_permiso.rol_id')->join('permiso','permiso.permiso_id','=','rol_permiso.permiso_id')->where('permiso_estado','=','1')->where('usuario_rol.user_id','=',Auth::user()->user_id)->orderBy('permiso_orden','asc')->get();
            ...
            $datos[$count]['deb'] = '';
            $datos[$count]['hab'] = '';
            $datos[$count]['act'] = Detalle_Diario::MayorProveedorAntCuenta($request->get('proveedorID'),$request->get('fecha_desde'),$request->get('cuenta_id'))->select(DB::raw('SUM(detalle_debe)-SUM(detalle_haber) as saldo'))->first()->saldo;
            if($datos[$count]['act'] == ''){
                $datos[$count]['act'] = 0;
            }
            $datos[$count]['dia'] = '';
            ...
            $datos[$count]['tot'] = '1';
            $count ++;
            foreach(Detalle_Diario::MayorProveedorCuenta($request->get('proveedorID'),$request->get('fecha_desde'),$request->get('fecha_hasta'),$request->get('cuenta_id'))->get() as $detalle){
            ...
            return view('admin.contabilidad.mayorProveedores.index',['cuentaC'=>$request->get('cuenta_id'),'cuentas'=>Cuenta::CuentasMovimiento()->get(),'proveedorC'=>$request->get('proveedorID'),'fDesde'=>$request->get('fecha_desde'),'fHasta'=>$request->get('fecha_hasta'),'datos'=>$datos,'proveedores'=>Proveedor::proveedores()->get(),'PE'=>Punto_Emision::puntos()->get(),'tipoPermiso'=>$tipoPermiso,'gruposPermiso'=>$gruposPermiso, 'permisosAdmin'=>$permisosAdmin]);      
        }catch(\Exception $ex){
            return redirect('mayorProveedores')->with('error2','Oucrrio un error en el procedimiento. Vuelva a intentar. ('.$ex->getMessage().')');
        }
    }
    public function pdf(Request $request){
        try{            
            ...
            $view =  \View::make('admin.formatosPDF.mayorProveedores', ['datos'=>$datos,'desde'=>DateTime::createFromFormat('Y-m-d', $request->get('fecha_desde'))->format('d/m/Y'),'hasta'=>DateTime::createFromFormat('Y-m-d', $request->get('fecha_hasta'))->format('d/m/Y'),'empresa'=>$empresa]);
