...
@section('principal')
    ...
    <div class="card card-secondary">
        ...
        <div class="card-header">
            <h3 class="card-title">¿Esta seguro de eliminar la Nota de Debito Bancaria?</h3>
            <div class="float-right">
                <form class="form-horizontal" method="POST" action="{{ route('listanotaDebitoBancario.destroy', [$notaDebito->nota_id]) }}">
                    @method('DELETE')
                    @csrf
                    <button type="submit" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i>&nbsp;Eliminar</button>
                    <!-- 
                    <button type="button" onclick='window.location = "{{ url("listanotaDebitoBancario") }}";' class="btn btn-default btn-sm"><i class="fa fa-undo"></i>&nbsp;Atras</button>
                    -->
                    ...
                    @if(isset($notaDebito)) 
                        @foreach($notaDebito->detallesTipoMovimiento as $x)
                            <tr>                           
                                ...
                                <td>@if($x->movimientond_tipo) {{ $x->movimientond_tipo}} @else CREDITO  @endif</td> 
