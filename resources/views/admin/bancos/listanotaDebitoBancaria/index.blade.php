...
@section('principal')
<form class="form-horizontal" method="POST" action="{{ url("listanotaDebitoBancario") }}">
@csrf
    <div class="card card-secondary">
        <div class="card-header">
            <h3 class="card-title">Lista de Notas de Debito Bancario</h3>                               
        </div>
        <div class="card-body">
                ...
                <tbody>                                                                        
                    @if(isset($notaDebito)) 
                        @foreach($notaDebito as $x)
                            ...
                            <td>
                                <a href="{{ url("listanotaDebitoBancario/{$x->nota_id}")}}" class="btn btn-xs btn-success" data-toggle="tooltip" data-placement="top" title="Visualizar"><i class="fa fa-eye"></i></a>    
                                @if(count($x->detallesTipoMovimiento) > 0)<a href="{{ url("listanotaDebitoBancario/{$x->nota_id}/eliminar")}}" class="btn btn-xs btn-danger"  data-toggle="tooltip" data-placement="top" title="Eliminar"><i class="fa fa-trash" aria-hidden="true"></i></a>@endif
                            </td>
                                <td>{{ $x->nota_numero}}</td>  
                                <td>{{ $x->nota_fecha}}</td>              
