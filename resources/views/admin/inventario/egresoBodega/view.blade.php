...
<div class="card card-secondary">
    <div class="card-header">
        <h3 class="card-title">Lista de Egresos de Bodega</h3>    
    </div>
    <!-- /.card-header -->
    <div class="card-body">
        <form class="form-horizontal" method="POST" action="{{ url("egresoBodega/buscar") }}">
        @csrf
                ...
                <tbody>
                    @if(isset($egreso))
                        ...
                        <tr>
                                ...
                                @if($x->cabecera_egreso_estado ==1)
                                    <td> 
                                    <input id="egreso_id" name="egreso_id" value="{{ $x->cabecera_egreso_id}}" type="hidden">   
                                    <a href="{{ url("egresoBodega/eliminar/{$x->cabecera_egreso_id}") }}" class="btn btn-xs btn-danger" data-toggle="tooltip" data-placement="top" title="Eliminar"><i class="fa fa-trash" aria-hidden="true"></i></a> 
                                    <a href="{{ url("egresoBodega/visualizar/{$x->cabecera_egreso_id}") }}" class="btn btn-xs btn-success" data-toggle="tooltip" data-placement="top" title="Ver"><i class="fa fa-eye"></i></a>    
                                </td> 
                                @else
                                <td> 
                                </td> 
                            ...
                            <td class="text-center"> <?php echo '$' . number_format($x->cabecera_egreso_subtotal, 2)?> </td>
                            <td class="text-center"> <?php echo '$' . number_format($x->cabecera_egreso_total, 2)?> </td>     
                            <td class="text-center" > 
                                <?php  
                                    if($x->cabecera_egreso_estado ==1){ echo 'Activo';}
                                ?>                      
                            </td>              
                        </tr>
                        @endforeach
